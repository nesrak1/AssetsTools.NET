...
namespace AssetsTools.NET
{
    public class TextureFile
    {
        ...
        public byte[] GetTextureData(string rootPath)
        {
            ...
            return DecodeTextureToBgra(pictureData, (TextureFormat)m_TextureFormat, m_Width, m_Height);
        }
        ...
        public byte[] GetTextureData(string rootPath, AssetBundleFile bundle)
        {
            if ((pictureData == null || pictureData.Length == 0) && m_StreamData.path != null && m_StreamData.path.StartsWith("archive:/") && bundle != null)
            {
                ...
                int resourceFileIndex = bundle.GetAssetsFileIndex(resourceFileName);
                if (resourceFileIndex >= 0)
                {
                    bundle.GetAssetsFileRange(resourceFileIndex, out long resourceFileOffset, out _);
                    pictureData = new byte[m_StreamData.size];
        ...
        public void SetTextureData(byte[] bgra)
        {
        ...
        {
            return DecodeTextureToBgra(data, format, width, height);
        }

        public static byte[] DecodeTextureToBgra(byte[] data, TextureFormat format, int width, int height)
        {
            switch (format)
            {
        ...
        public static byte[] EncodeTextureFromBgra(byte[] data, TextureFormat format, int width, int height)
        {
            switch (format)
            {
                case TextureFormat.R8:
                    return RGBAEncoders.EncodeR8(data, width, height);
                case TextureFormat.R16:
                    return RGBAEncoders.EncodeR16(data, width, height);
                case TextureFormat.RG16:
                    return RGBAEncoders.EncodeRG16(data, width, height);
                case TextureFormat.RGB24:
                    return RGBAEncoders.EncodeRGB24(data, width, height);
                case TextureFormat.RGB565:
                    return RGBAEncoders.EncodeRGB565(data, width, height);
                case TextureFormat.RGBA32:
                    return RGBAEncoders.EncodeRGBA32(data, width, height);
                case TextureFormat.ARGB32:
                    return RGBAEncoders.EncodeARGB32(data, width, height);
                case TextureFormat.RGBA4444:
                    return RGBAEncoders.EncodeRGBA4444(data, width, height);
                case TextureFormat.ARGB4444:
                    return RGBAEncoders.EncodeARGB4444(data, width, height);
                case TextureFormat.Alpha8:
                    return RGBAEncoders.EncodeAlpha8(data, width, height);
                case TextureFormat.RHalf:
                    return RGBAEncoders.EncodeRHalf(data, width, height);
                case TextureFormat.RGHalf:
                    return RGBAEncoders.EncodeRGHalf(data, width, height);
                case TextureFormat.RGBAHalf:
                    return RGBAEncoders.EncodeRGBAHalf(data, width, height);
                default:
                    return null;
            }
        }
