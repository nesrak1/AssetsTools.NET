...
//    catch (Exception e)
//    {
//        Console.WriteLine($"failed to encode {format}: {e.Message}");
//    }
//}

var am = new AssetsManager();
var bun = am.LoadBundleFile("newimg_DXT5.unity3d");
var assetsInst = am.LoadAssetsFileFromBundle(bun, "CAB-texture-DXT5");
var textureInf = assetsInst.file.GetAssetsOfType(AssetClassID.Texture2D)[0];
var texture = TextureFile.ReadTextureFile(am.GetBaseField(assetsInst, textureInf));
using var writeStream = File.OpenWrite("test_dxt5_out.png");
texture.SaveTextureDataToImage(assetsInst, writeStream, ImageExportType.Png);
